...
describe("チャネル連携ページ", () => {
  ...
  it("連携チャネルが配信中ならヘッダーのロゴに配信中バッジが付く", async () => {
    const env0 = makeEnv();
    const { cookie } = await loginAs(env0);
    await env0.DB.prepare(
      `INSERT INTO connections
    ...
    expect(await offline.text()).not.toContain("brand-orb brand-live");

    await env0.DB.prepare(
      "INSERT INTO live_streams (twitch_channel_id, stream_id) VALUES ('12345', 'stream-1')",
    ).run();

    // 配信中はヘッダーとフッターの2箇所にバッジが付く。
    const live = await fetchAs(
      env0,
      new Request("https://example.com/channels", {
        headers: { Cookie: cookie },
      }),
    ...
    expect(body).toContain('aria-label="orbsky トップ(配信中)"');
    // ヘッダーの小さいエンブレムでは文字なしのドットを使う。
    expect(body).not.toContain(">LIVE<");
  });

  it("他ユーザーのチャネルが配信中でも自分のロゴにはバッジが付かない", async () => {
    const env0 = makeEnv();
    const { cookie } = await loginAs(env0);
    await env0.DB.prepare(
      `INSERT INTO connections
    ...
    await env0.DB.prepare(
      "INSERT INTO live_streams (twitch_channel_id, stream_id) VALUES ('99999', 'stream-x')",
    ).run();
    ...
    expect(res.status).toBe(200);
    expect(await res.text()).not.toContain("brand-orb brand-live");
  });
});
